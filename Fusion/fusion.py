...
def inference_single_image(thermal_path, ir_path, model_path, output_path, device='cuda'):
    """单张图像推理函数"""
    # 初始化模型
    model = EnhancedFusionModel().to(device)
    ...
    if thermal is None or ir is None:
    ...
    # 转换为Tensor
    thermal_tensor = torch.from_numpy(thermal).float().unsqueeze(0).unsqueeze(0) / 255.0
    ir_tensor = torch.from_numpy(ir).float().unsqueeze(0).unsqueeze(0) / 255.0

    # 推理
    with torch.no_grad():
        fused = model(thermal_tensor.to(device), ir_tensor.to(device))
...
# def inference_batch(thermal_dir, ir_dir, model_path, output_dir, device='cuda'):
#     """批量推理函数"""
#     # 初始化模型
#     model = EnhancedFusionModel().to(device)
#     model.load_state_dict(torch.load(model_path))
...
#
#     # 获取文件列表
#     thermal_files = sorted([f for f in os.listdir(thermal_dir) if f.endswith(('.png','.jpg'))])
#     ir_files = sorted([f for f in os.listdir(ir_dir) if f.endswith(('.png','.jpg'))])
#
#     # 创建输出目录
#     os.makedirs(output_dir, exist_ok=True)
#
#     # 批量处理
#     for thermal_name, ir_name in tqdm(zip(thermal_files, ir_files), total=len(thermal_files)):
#         thermal_path = os.path.join(thermal_dir, thermal_name)
...
#         # 保存结果
#         fused_img = (fused.squeeze().cpu().numpy() * 255).astype(np.uint8)
#         output_path = os.path.join(output_dir, f"fused_{thermal_name}")
#         cv2.imwrite(output_path, fused_img)
